...
public class MercatorSieveTest {
	...
	public void testMultiThreadedTestSequentialDequeue() throws InterruptedException, IOException {
		...
		final File tempDir = Files.createTempDirectory("temp-"+ UUID.randomUUID()).toFile();
	...
	public void testSimple() throws IOException, InterruptedException {
		...
		sieve.enqueue("A0", 0);
		sieve.enqueue("A1", 1);
		sieve.enqueue("A0", 3); // Will not be dequeued
		sieve.enqueue("A3", 2);
		...
		sieve.enqueue("A5", 1);
		sieve.enqueue("A2", 0);
		sieve.enqueue("A2", 3); // Will not be dequeued
		sieve.enqueue("A5", 1);
		sieve.enqueue("A6", 2);
		Thread.sleep(2000); // Give time to the dequeueing thread.
		sieve.flush();
