 ...
 * whereas {@linkplain #enqueueRelativePageInfo(byte[]) enqueing} is performed by the {@link Distributor}.
 *
...
public class VisitState implements Delayed, Serializable {
	...
	static {
		...
		ROBOTS_PATH = FrontierProtobuf.RelativePageInfo.newBuilder().setUrlPathQuery(ByteString.copyFrom(robotsString)).build().toByteArray();
	}
	...
	private final transient ObjectArrayFIFOQueue<byte[]> relativePageInfo;
	...
	public VisitState(final byte[] schemeAuthority) {
		...
		relativePageInfo = new ObjectArrayFIFOQueue<>();
	...
	public synchronized void enqueueRobots() {
		...
		synchronized(this) {
			if (relativePageInfo.isEmpty()) {
				relativePageInfo.enqueueFirst(ROBOTS_PATH);
				putInEntryIfNotAcquired();
			}
			else {
				final byte[] first = relativePageInfo.dequeue();
				relativePageInfo.enqueueFirst(ROBOTS_PATH);
				relativePageInfo.enqueueFirst(first);
			}
		}
	}
	...
	void removeRobots() {
		// Nothing to remove
		if (relativePageInfo.isEmpty()) return;

		// We test the first path
		final byte[] firstPath = relativePageInfo.dequeue();
		...
		if (isEmpty()) {
			...
			relativePageInfo.enqueueFirst(firstPath);
			return;
		}

		// We test the second path
		final byte[] secondPath = relativePageInfo.dequeue();
		...
		relativePageInfo.enqueueFirst(secondPath);
		relativePageInfo.enqueueFirst(firstPath);
	}
	 ...
	 *
	 * @param relativePageInfo a path+query in byte-array representation.
	 */
	public void enqueueRelativePageInfo(final byte[] relativePageInfo) {
		synchronized (this) {
			if (nextFetch == Long.MAX_VALUE) return;
			final boolean wasEmpty = this.relativePageInfo.isEmpty();
			this.relativePageInfo.enqueue(relativePageInfo);
			if (wasEmpty) putInEntryIfNotAcquired();
		}
		Frontier frontier = Agent.getFrontier();
		frontier.pathQueriesInQueues.incrementAndGet();
		frontier.weightOfpathQueriesInQueues.addAndGet(BURL.memoryUsageOf(relativePageInfo));
	}
	...
	public byte[] dequeue() {
		final byte[] array;
		synchronized (this) {
			array = relativePageInfo.dequeue();
		}
		Frontier frontier = Agent.getFrontier();
	...
	public synchronized void clear() {
		while(! isEmpty()) dequeue(); // We cannot invoke relativePageInfo.clear(), as counters would not be updated.
		// TODO: we should find a way to release more resources.
		relativePageInfo.trim();
	}
	...
	public synchronized boolean isEmpty() {
		return relativePageInfo.isEmpty();
	}
	...
	private void writeObject(java.io.ObjectOutputStream s) throws java.io.IOException {
		...
		int size = relativePageInfo.size();
		s.writeInt(size);

		while(size-- != 0) Util.writeByteArray(relativePageInfo.dequeue(), s);
	}
	...
	private void readObject(java.io.ObjectInputStream s) throws java.io.IOException, ClassNotFoundException, NoSuchFieldException, SecurityException, IllegalArgumentException, IllegalAccessException {
		...
		Field field = getClass().getDeclaredField("relativePageInfo");
		field.setAccessible(true);
		field.set(this, new ObjectArrayFIFOQueue<byte[]>(size));

		while(size-- != 0) relativePageInfo.enqueue(Util.readByteArray(s));
	}
